...
// Handler для выполнения транзакции
...
// Имя запроса используется для логирования и потенциально может использоваться ещё где-то, например, для трейсинга
type Query struct {
	Name     string
	QueryRaw string
}
...
// Pinge интерфейс ддля проверки соединения с БД
